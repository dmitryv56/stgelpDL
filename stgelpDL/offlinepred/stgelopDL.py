...
from offlinepred.drive import drive, drive_hmaggregated
...
def parser()->(argparse.ArgumentParser, str):
 ...
 # command-line parser
    ...
    shoursHelp          = "Daylight hours. A string contains the comma-separated begin and end daylight hours. " + \
                          "Applicable only in 'hmofflp'-mode. The time series should being aggregated along hours " +\
                          "in this diapozon."
    sminuteHelp         = "Together with the hour from the range of daylight hours, it indicates the exact time "+\
                          "direction hh:mm along should being carryed the aggregation out. Applicable only in " +\
                          "hmofflp-mode."
    ...
    smodeHelp           = "Possible modes: 'offlp' - offline predict, " + \
                          "'hmoofl' -hour,minute aggregation, offline predictor"

    parser = argparse.ArgumentParser(description=sDescriptor)

    parser.add_argument('-m', '--mode', dest='cl_mode', action='store', default='offlp', choices=['offlp','hmofflp'],
                        help=smodeHelp)
    ...
    parser.add_argument(      '--hours', dest='cl_hours',   action='store', default='7,19',    help=shoursHelp)
    parser.add_argument(      '--minute', dest='cl_minute', action='store', default=0,        help=sminuteHelp)
...
Daylight hours                     : {args.cl_hours}    ** 'hmofflp' mode only
Minute                             : {args.cl_minute}   ** 'hmofflp' mode only
...
def logParams(param:tuple)->str:
    (title, mode, csv_source, dt_col_name, ts_list, discret, n, n_step, n_eval, n_test, n_pred, daylight, daylightend, \
     minute, folder_for_logging, folder_for_train_logging, folder_for_control_logging, folder_for_predict_logging) \
        = param
...
Daylight                           : {daylight} - {daylightend}   ** hmofflp - mode only.
Minute                             : {minute}                     ** hmofflp - mode only.
...
def main(argc,argv):
    ...
    h_list      = list(args.cl_hours.split(','))
    minute      = int(args.cl_minute)
    daylight    = int(h_list[0])
    daylightend = int(h_list[1])
    ...
    try:
        ...
        param=(title ,mode, csv_source,dt_col_name,ts_list,discret,n, n_step,n_eval,n_test,n_pred,daylight, daylightend,
               minute, folder_for_logging, folder_for_train_logging, folder_for_control_logging,
               folder_for_predict_logging)
    ...
    try:
        if mode=="offlp":
            ...
            ret =drive_hmaggregated(df= df, title=title, dt_col_name=dt_col_name, ts_list=ts_list, discret=discret,n=n,
                                    n_step=n_step, n_eval=n_eval, n_test=n_test, n_pred=n_pred,
                                    daylight_begin=daylight, daylight_end=daylightend, minute=minute,
                                    folder_for_logging=folder_for_logging,
