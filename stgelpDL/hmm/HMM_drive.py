...
from predictor.cfg import MAGIC_SEED, PATH_DATASET_REPOSITORY, DT_DSET, RCPOWER_DSET, DISCRET, LOG_FILE_NAME, \
...
from hmm.HMM import hmm_gmix, hmm_dobs
...
def trainDataset(cp: ControlPlane, source_folder: str, LISTCSV: list )->(list, np.array, list, np.array, np.array,np.array):
    """
...
def trainHMMprob(concat_list: list, states_concat_list: list , cp: ControlPlane)->(np.array,np.matrix,np.matrix):
    ...
    if (len(ar_state) - 64) >=0:
        plotArray(ar_state[len(ar_state)-64:], "Train Hidden States for hmm (last items)", "hiddenStates_HMM_last", cp)
    del ar_state
...
def initialDist(concat_list:list, states: np.array, valBound:float, f:object = None)->np.array:
    ...
    for i in range(n_samples):
        val = concat_list[i][0]
        if val< (-valBound): aux[i]=- 1
        elif val>=(valBound): aux[i]=1
        else:  pass
...
def emissionDistribution(trainArray: np.array, ar_state: np.array,states: np.array, counts: np.array,f: object)->np.array:
    pass
...
def boundBalance(std: float)->float:
    return 0.1 * std
#
def calcTransitions(cp: ControlPlane, df: pd.DataFrame, x: np.array, std: float, signed_imbalance: str, f: object)->(pd.DataFrame, np.array, dict, dict,np.array):
    """
