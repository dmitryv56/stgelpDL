...
ddiffseas = lambda x, d: [x[i] - 2 * x[i + 1] + x[i + 2] - x[i + d] + 2 * x[i + d + 1] - x[i + d + 2] for i in
                          range(len(x) - d - 2)]
diff1seas1d = lambda x, d: [x[i] - x[i + 1] - x[i + 72] + x[i + 72 + 1] for i in range(len(x) - d - 1)]
ddiff1seas1d = lambda x, d: [x[i] - 2 * x[i + 1] + x[i + 2] - x[i + 72] + 2 * x[i + 72 + 1] - x[i + 72 + 2] for i in
                             range(len(x) - d - 1)]
seas288 = lambda x, d: [
    x[i] - x[i + 24] - x[i + 2 * 24] + x[i + 4 * 24] + x[i + 5 * 24] + x[i + 7 * 24] + x[i + 8 * 24] - x[i + 10 * 24] \
...
def seasFilter(name: str, l_seas: dict, df: pd.DataFrame, main_col_name: str, discret: int, NFFT: int,
               f: object) -> None:
    if len(l_seas) == 0:
        return
