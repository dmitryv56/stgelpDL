...
def getItems(section_: str, dict_items: dict = None) -> tuple:
    parser = getparserobj()
    res = []
    for item, typ in dict_items.items():
        if typ == 'str':
            res.append(parser[section_][item])
        elif typ == 'int':
            res.append(int(parser[section_][item]))
        elif typ == 'float':
            res.append(float(parser[section_][item]))
        elif typ == 'bool':
            res.append(bool(parser[section_][item]))
        elif typ == 'eval':
            res.append(eval(parser[section_][item]))
        elif typ == 'None':
            res.append(None)
        else:
            res.append(parser[section_][item])
...
(FILE_DESCRIPTOR, SCALED_DATA_FOR_AUTO, START_DATE_FOR_AUTO, END_DATE_FOR_AUTO, TIME_TRUNC_FOR_AUTO, GEO_LIMIT_FOR_AUTO,
    ...
    'START_DATE_FOR_AUTO': 'str', 'END_DATE_FOR_AUTO': 'str', 'TIME_TRUNC_FOR_AUTO': 'str',
