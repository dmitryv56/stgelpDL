...
class tsARIMA(Statmodel):
    ...
    @staticmethod
    def set_SARIMA(val):
        (tsARIMA._p_ARIMA, tsARIMA._d_ARIMA, tsARIMA._q_ARIMA, tsARIMA._P_ARIMA, tsARIMA._D_ARIMA,
         tsARIMA._Q_ARIMA) = val
    ...
    def control_arima(self):
        ...
        if (self.nameModel == 'control_best_arima'):
            ...
            model = pm.auto_arima(self.ts_data, exogenous=None, d=0, max_d=2, start_q=0, max_q=0, seasonal=False,
                                  trace=True, \
                                  error_action='ignore', suppress_warnings=True, stepwise=True)
        ...
        if (self.nameModel == 'control_seasonal_arima'):
            ...
            model = pm.auto_arima(self.ts_data, start_p=0, max_p=1, start_q=0, max_q=1, d=0, max_d=1, D=0, max_D=0,
                                  start_P=0, max_P=1, start_Q=0, max_Q=0,
    ...
    def fitted_model_logging(self):
        ...
        filePng = Path(PlotPrintManager.get_PredictLoggingFolder()) / (
            "Predict_{}_{}.png".format(self.nameModel, self.timeseries_name))
...
def predict_arima(ds, cp, n_predict):
    rcpower = ds.df[cp.rcpower_dset].values
    model = pm.auto_arima(rcpower, start_p=1, start_q=1, max_p=3, max_q=3, seasonal=False, d=1, trace=True,
                          stepwise=True)
    predict = model.fit_predict(rcpower, None, n_predict)
